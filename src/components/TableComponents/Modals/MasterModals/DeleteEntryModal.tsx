...
export default function DeleteEntryModal(props: DeleteEntryModalProp) {
  const deleteRoute = api.user.deleteUser.useMutation({
    ...
    onSuccess: () => {
      toast.success("Successfully deleted " + props.item + ".")
    }
  });
              ...
              <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                >
                  <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                      <CardTitle heading={"Delete " + props.item + "..."} subheading={"Are you sure you want to delete this " + props.item + "? This action cannot be undone."}></CardTitle>
                      <div className="gap-5 flex flex-row justify-around px-4 py-4 sm:px-6">
                        ...
                        <PrimaryButton onClick={handleDelete}
                                       buttonText={"Delete " + props.item}></PrimaryButton>
                      </div>
                  </Dialog.Panel>
                </Transition.Child>
              </div>
